...
describe('pluginAddEntry', () => {
  ...
  it('wraps hydration entry fallback behind host init during build', async () => {
    ...
    expect(result?.code.indexOf('await __mfPendingShares.preloadPendingShares();')).toBeGreaterThan(
      result?.code.indexOf('await initHost();') ?? -1
    );
    expect(result?.code.indexOf('await __mfPendingShares.preloadPendingShares();')).toBeLessThan(
